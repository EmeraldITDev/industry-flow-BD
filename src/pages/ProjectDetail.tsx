...
export default function ProjectDetail() {
      ...
      const mappedProject: Project = {
        ...
        subproducts: Array.isArray(data.subproducts)
          ...
          : Array.isArray(data.sub_products)
            ...
            : (data.sub_product ?? data.subProduct)
              ? [String(data.sub_product ?? data.subProduct)]
              : [],
        channelPartner: data.channel_partner || data.channelPartner,
        contractValueNGN:
          parseFloat(data.contractValueNGN ?? data.contract_value_ngn) || 0,
        contractValueUSD:
          parseFloat(data.contractValueUSD ?? data.contract_value_usd) || 0,
        marginPercentNGN:
          parseFloat(data.marginPercentNGN ?? data.margin_percent_ngn) || 0,
        marginPercentUSD:
          parseFloat(data.marginPercentUSD ?? data.margin_percent_usd) || 0,
        marginValueNGN:
          parseFloat(data.marginValueNGN ?? data.margin_value_ngn) ||
          (parseFloat(data.contractValueNGN ?? data.contract_value_ngn) &&
          parseFloat(data.marginPercentNGN ?? data.margin_percent_ngn)
            ? (parseFloat(data.contractValueNGN ?? data.contract_value_ngn) *
                parseFloat(data.marginPercentNGN ?? data.margin_percent_ngn)) /
              100
            : 0),
        marginValueUSD:
          parseFloat(data.marginValueUSD ?? data.margin_value_usd) ||
          (parseFloat(data.contractValueUSD ?? data.contract_value_usd) &&
          parseFloat(data.marginPercentUSD ?? data.margin_percent_usd)
            ? (parseFloat(data.contractValueUSD ?? data.contract_value_usd) *
                parseFloat(data.marginPercentUSD ?? data.margin_percent_usd)) /
              100
            : 0),
        ...
        teamMemberIds: (data.team_member_ids || data.teamMemberIds || []).map(
          (id: any) => String(id),
        ),
      };
  ...
  const handleStatusChange = async (newStatus: ProjectStatus) => {
    ...
    if (
      ...
      !isValidStageStatus(project.pipelineStage, newStatus)
    ) {
      const allowed = getAllowedStatusesForStage(project.pipelineStage)
        .map(getStatusLabel)
        .join(", ");
      toast.error(
        `Cannot set "${getStatusLabel(newStatus)}" for stage "${project.pipelineStage}". Allowed: ${allowed}`,
      );
      return;
    }
    ...
    <div className="p-4 sm:p-6 lg:p-8 space-y-4 sm:space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-4">
        ...
        <div className="flex-1 min-w-0">
          ...
          <h1 className="text-lg sm:text-2xl lg:text-3xl font-bold line-clamp-2">
            {project.name}
          </h1>
        </div>
        {canEditProjects && (
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              cannot be undone and will permanently remove all associated tasks
              and data.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            ...
            <CardContent className="p-3 sm:p-6 pt-0">
              ...
              <div className="mt-3 sm:mt-4 grid grid-cols-2 md:grid-cols-3 gap-3 sm:gap-4 pt-3 sm:pt-4 border-t">
                ...
                {(project.marginPercentNGN !== undefined ||
                  project.marginValueNGN !== undefined) && (
                  <div>
                    <p className="text-xs text-muted-foreground">
                      Margin % (NGN)
                    </p>
                    <p className="font-medium text-sm">
                      {typeof project.marginPercentNGN === "number" &&
                      project.marginPercentNGN > 0
                    ...
                    {project.marginValueNGN !== undefined &&
                      project.marginValueNGN > 0 && (
                        <p className="font-medium text-sm">
                          ₦{Number(project.marginValueNGN).toLocaleString()}
                        </p>
                      )}
                  </div>
                )}
                ...
                {(project.marginPercentUSD !== undefined ||
                  project.marginValueUSD !== undefined) && (
                  <div>
                    <p className="text-xs text-muted-foreground">
                      Margin % (USD)
                    </p>
                    <p className="font-medium text-sm">
                      {typeof project.marginPercentUSD === "number" &&
                      project.marginPercentUSD > 0
                    ...
                    {project.marginValueUSD !== undefined &&
                      project.marginValueUSD > 0 && (
                        <p className="font-medium text-sm">
                          ${Number(project.marginValueUSD).toLocaleString()}
                        </p>
                      )}
                  </div>
                )}
                ...
                {project.pipelineStage && (
                  <p className="text-xs text-muted-foreground mt-2">
                    ...
                    {PIPELINE_STAGES.find(
                      (s) => s.value === project.pipelineStage,
                    )?.label || project.pipelineStage}
                  </p>
                )}
              </div>
            ...
            <CardContent className="space-y-3">
              {(() => {
                const lead = teamMembers.find(
                  (m: any) =>
                    m.id === project.projectLeadId ||
                    String(m.id) === String(project.projectLeadId),
                );
                const assignee = project.assigneeId
                  ? teamMembers.find(
                      (m: any) =>
                        m.id === project.assigneeId ||
                        String(m.id) === String(project.assigneeId),
                    )
                  : null;

                // Get team members excluding lead and assignee
                const leadId = project.projectLeadId
                  ? String(project.projectLeadId)
                  : null;
                const assigneeIdStr = project.assigneeId
                  ? String(project.assigneeId)
                  : null;
                const otherMembers = (project.teamMemberIds || [])
                  .filter((id) => id !== leadId && id !== assigneeIdStr)
                  .map((id) =>
                    teamMembers.find((m: any) => String(m.id) === id),
                  )
                  .filter(Boolean);
                  ...
                  <>
                    ...
                    <div className="pt-2 border-t border-border">
                      {(() => {
                        ...
                        const displaySize =
                          dynamicSize || project.teamSize || 0;
                        return (
