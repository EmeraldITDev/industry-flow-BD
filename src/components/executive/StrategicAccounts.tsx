...
export function StrategicAccounts({ data, onManageGroups }: Props) {
  ...
  const drillFor = (account: AccountSnapshot) =>
    `clientNames=${encodeURIComponent(JSON.stringify(account.clientNames))}`;
            ...
            <ScrollArea className="h-full">
              <div className="p-6 space-y-5">
                ...
                <div className="grid gap-3 grid-cols-2">
                  <ExecutiveMetric label="Opportunities" value={String(selected.count)} drillTo={drillFor(selected)} />
                  <ExecutiveMetric
                    label="Won / in execution"
                    value={String(selected.won)}
                    sub={`${fmtUsd(selected.wonUsd)} · ${fmtNgn(selected.wonNgn)}`}
                  />
                  <ExecutiveMetric label="Pipeline (USD)" value={fmtUsd(selected.usd)} />
                  <ExecutiveMetric label="Pipeline (NGN)" value={fmtNgn(selected.ngn)} />
                  <ExecutiveMetric label="In proposal" value={String(selected.proposal)} />
                  <ExecutiveMetric label="In negotiation" value={String(selected.negotiation)} />
                  <ExecutiveMetric
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2 mt-2">
                    {(['high', 'medium', 'low'] as const).map((band) => (
                      <Badge key={band} variant="outline" className="font-normal capitalize">
                ...
                {selected.byEntity.length > 1 && (
                  <div>
                    ...
                    <RankedList groups={selected.byEntity} filterKey="clientNames" limit={6} />
                  </div>
                )}
                ...
                <Button variant="outline" className="w-full" onClick={() => navigate(`/projects?${drillFor(selected)}`)}>
